...
def test_sbox_leakage_calculation():
    ...
    model = SboxOutputLeakageModel(plaintexts)

    # Calculate for byte_location 0, key guess 0x12
    result = model.calculate(byte_location=0, key_guess=0x12)
...
def test_vectorization_multiple_traces():
    ...
    results = model.calculate(byte_location=1, key_guess=guess)
...
def test_wrong_guess_produces_different_leakage():
    ...
    leakage_guess_1 = model.calculate(0, key_guess=0x01)
    leakage_guess_2 = model.calculate(0, key_guess=0x02)
